    ...
    try {
      const res = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          ...
          subject: "New enquiry — W. Safe Finance",
          from_name: "W. Safe Finance Form",
          name: values.name,
          email: values.email,
          phone: values.phone || "—",
          topic: values.topic || "—",
            ...
            <div className="mt-10 rounded-2xl border border-line/80 bg-white p-6 sm:p-8 dark:border-white/10 dark:bg-midnight">
              {status === "error" ? (
                <div className="max-w-xl">
                  <div className="text-xs font-medium tracking-[0.18em] text-red-500">Error</div>
                  <h2 className="mt-3 font-serif text-2xl text-ink dark:text-paper">Could not send message</h2>
                  <p className="mt-3 text-sm leading-relaxed text-slateText/75 dark:text-paper/70">
                    Something went wrong. Please try again or contact us directly: 782-002-822.
                  </p>
                  <div className="mt-6">
